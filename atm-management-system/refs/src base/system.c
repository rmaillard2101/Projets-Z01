...
void makeTransaction(struct User u) {
  ...
  char name[100];         // pour getAccountFromFile
  char receiverName[100]; // nom saisi du destinataire
  ...
  printf("Entrez le numéro de votre compte (expéditeur) : ");
  ...
  if (!senderFound) {
    fclose(fp);
    fclose(temp);
    printf("\n✖ Compte expéditeur introuvable ou non associé à votre "
           "utilisateur.\n");
  ...
  if (strcmp(sender.accountType, "fixed01") == 0 ||
    ...
    fclose(temp);
    printf(
        "\n✖ Ce compte est à terme (fixed), les transferts sont interdits.\n");
    stayOrReturn(0, makeTransaction, u);
    return;
  }
  ...
  // Revenir au début du fichier original pour le réécrire
  rewind(fp);
  ...
  while (getAccountFromFile(fp, name, &r)) {
    ...
    } else if (r.accountNbr == receiver.accountNbr &&
